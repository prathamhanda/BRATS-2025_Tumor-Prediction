...
Pediatric brain tumor segmentation poses distinct challenges compared to adult glioma segmentation, including increased anatomical variability and often extremely small enhancing tumor regions. We present a research‑grade, end‑to‑end segmentation pipeline for BraTS Pediatric (PEDs) subregion segmentation built on nnU‑Net v2 (3D full‑resolution) with multi‑fold ensembling, explicit label‑ontology auditing, conservative inference settings for remote GPU stability, post‑processing to suppress spurious components, and automated visual quality control (QC) exports. A key system component is a label‑audit stage that detects the enhancing tumor (ET) label convention (ET=3 vs ET=4) and propagates it consistently across region definitions, post‑processing, and evaluation—preventing silent metric errors. On internal evaluation using 260 labeled training cases, we achieve mean Dice of 0.9502 (WT), 0.7995 (TC), and 0.8052 (ET), corresponding to a macro‑average Dice of 0.8516 across the three reported regions. The final system produces challenge‑compliant submissions and an inspection-ready visualisations bundle (PNG overlays, histograms, and HTML galleries) to support rapid reviewer and judge verification.
